...
const Recent = () => {
    ...
    const fetchRecentItems = async () => {
        try {
            ...
            // 获取最近添加的内容
            const response = await apiClient.get(`/Users/${userId}/Items/Latest`, {
                params: {
                    Limit: 20,
            ...
            console.error('获取最近添加项目失败:', error);
            message.error('获取最近添加项目失败，请检查网络连接');
            setLoading(false);
        }
    };
    ...
    return (_jsxs("div", { className: "recent-container", children: [_jsx("h2", { children: "\u6700\u8FD1\u6DFB\u52A0" }), loading ? (_jsx("div", { className: "loading-container", children: _jsx(Spin, { size: "large" }) })) : mediaItems.length === 0 ? (_jsx(Empty, { description: "\u6682\u65E0\u6700\u8FD1\u6DFB\u52A0\u5185\u5BB9" })) : (_jsx(Row, { gutter: [16, 16], children: mediaItems.map((item) => (_jsx(Col, { xs: 12, sm: 8, md: 6, lg: 4, children: _jsx(Card, { hoverable: true, cover: _jsx("img", { alt: item.Name, src: getImageUrl(item) }), onClick: () => handleItemClick(item), children: _jsx(Card.Meta, { title: item.Name, description: item.ProductionYear ? `${item.ProductionYear}` : '' }) }) }, item.Id))) }))] }));
};
